...
async def handle_asr_got_query(
    ...
    Forwards the query and parameters to the GoTProcessor, optionally including the graph state and a reasoning trace summary in the response. Handles conversion and validation of returned data, measures execution time, and provides robust error handling with fallback responses if processing fails.
