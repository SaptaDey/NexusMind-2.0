...
async def handle_asr_got_query(
    ...
    Handles the "asr_got.query" method by forwarding the query and parameters to the GoTProcessor. Optionally includes the graph state and a reasoning trace summary in the response, depending on the request parameters. Converts and validates returned data, measures execution time, and provides robust error handling with fallback responses if processing fails.
    
    Returns:
        A JSON-RPC response containing the ASR-GoT query result, which includes the answer, optional reasoning trace summary, graph state, confidence vector, execution time, and session ID. On error, returns a JSON-RPC error response with details.
    """
    logger.info(
