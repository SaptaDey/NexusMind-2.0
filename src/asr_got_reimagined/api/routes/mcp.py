...
async def mcp_endpoint_handler(
    ...
    Parses the method and parameters from the incoming JSON-RPC request, then invokes the corresponding handler for "initialize", "asr_got.query", or "shutdown". Returns a JSON-RPC response with the result or an error response for unsupported methods, invalid parameters, or internal exceptions.
    """
    logger.debug(
