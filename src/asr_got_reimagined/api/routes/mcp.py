...
async def handle_asr_got_query(
    ...
    Returns:
        A JSON-RPC response containing the ASR-GoT query result, which may include the answer, reasoning trace summary, graph state, confidence vector, execution time in milliseconds, and session ID. If an error occurs, returns a JSON-RPC error response with relevant details.
    """
    logger.info(
