...
async def mcp_endpoint_handler(
    ...
    Parses the method and parameters from the incoming JSON-RPC request, invokes the corresponding handler for "initialize", "asr_got.query", or "shutdown", and returns a JSON-RPC response. Returns a JSON-RPC error response for unsupported methods or invalid parameters. Exceptions are logged and mapped to standard JSON-RPC error codes.
    """
    logger.debug(
