...
async def mcp_endpoint_handler(
    ...
    Parses the method and parameters from the incoming JSON-RPC request, then routes the request to the handler for "initialize", "asr_got.query", or "shutdown". Returns a structured JSON-RPC response or an error if the method is unsupported or parameters are invalid. Exceptions are logged and mapped to standard JSON-RPC error responses.
    """
    logger.debug(
