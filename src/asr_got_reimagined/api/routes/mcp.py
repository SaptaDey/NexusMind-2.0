...
async def handle_asr_got_query(
    ...
    Handles the "asr_got.query" method by forwarding the query and parameters to the GoTProcessor. Optionally includes graph state and reasoning trace in the response, converting and validating returned data as needed. Measures execution time and provides robust error handling, including fallback responses in case of processing failures.
    
    Returns:
        A JSON-RPC response containing the ASR-GoT query result, which may include the answer, reasoning trace summary, graph state, confidence vector, execution time, and session ID. On error, returns a JSON-RPC error response with details.
    """
    logger.info(
...
async def mcp_endpoint_handler(
    ...
    Parses the method and parameters from the incoming JSON-RPC request, then invokes the corresponding handler for "initialize", "asr_got.query", or "shutdown". Returns a JSON-RPC error response for unsupported methods or invalid parameters. Exceptions are logged and mapped to standard JSON-RPC error responses.
    """
    logger.debug(
