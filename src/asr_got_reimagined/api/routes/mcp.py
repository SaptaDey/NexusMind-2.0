...
async def mcp_endpoint_handler(
    ...
    Parses the method and parameters from the incoming JSON-RPC request, then invokes the corresponding handler for "initialize", "asr_got.query", or "shutdown". Returns a structured JSON-RPC response or an error response for unsupported methods or invalid parameters. Exceptions are logged and mapped to standard JSON-RPC error codes.
    """
    logger.debug(
