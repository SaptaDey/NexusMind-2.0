...
async def handle_asr_got_query(
    ...
    """
    Processes an ASR-GoT query request and returns the result or an error response.
    
    Forwards the query and parameters to the GoTProcessor, optionally including graph state and reasoning trace in the response. Handles conversion and validation of returned data, measures execution time, and provides robust error handling with fallback responses if processing fails.
    
    Returns:
        A JSON-RPC response containing the ASR-GoT query result, including the answer, optional reasoning trace summary, graph state, confidence vector, execution time, and session ID. If an error occurs, returns a JSON-RPC error response with details.
    """
    logger.info(
