...
class JSONRPCResponse(BaseModel, Generic[T, E]):
    ...
    def check_result_error_conditions(
        ...
        """
        Ensures that a JSON-RPC response includes either a result or an error, but not both.
        
        Raises:
            ValueError: If both "result" and "error" are provided, or if neither is present.
