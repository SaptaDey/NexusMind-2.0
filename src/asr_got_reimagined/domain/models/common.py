...
def _validate_probability_distribution(v: list[float]) -> list[float]:
    ...
    Allows empty lists and does not require the values to sum to 1.0, assuming normalization may occur elsewhere. Raises a ValueError if any value is outside the valid range.
    
    Returns:
        The validated list of floats.
    
    Raises:
        ValueError: If any value is not between 0.0 and 1.0.
    """
...
class ConfidenceVector(BaseModel):
    ...
    def from_list(cls, values: list[float]) -> "ConfidenceVector":
        ...
        Returns:
            ConfidenceVector instance with fields set from the provided list.
