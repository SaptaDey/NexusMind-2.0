...
def _validate_probability_distribution(v: list[float]) -> list[float]:
    ...
    Checks that all values are between 0.0 and 1.0. Allows empty lists and does not enforce that the sum of probabilities is exactly 1.0, permitting non-normalized or uninitialized distributions.
    
    Args:
        v: List of float values to validate.
...
class ConfidenceVector(BaseModel):
    ...
    def to_list(self) -> list[float]:
        """
        Returns the confidence vector as a list of four float values in fixed order.
        
        The returned list contains empirical support, theoretical basis, methodological rigor,
        and consensus alignment, in that order.
        """
        return [
