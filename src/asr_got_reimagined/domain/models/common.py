...
def _validate_probability_distribution(v: list[float]) -> list[float]:
    ...
    Checks that all values are between 0.0 and 1.0, allowing empty lists and not enforcing that the sum equals 1.0. Raises a ValueError if any value is outside the valid range.
    """
...
class ConfidenceVector(BaseModel):
    ...
    def to_list(self) -> list[float]:
        ...
        The returned list contains, in order: empirical support, theoretical basis,
        methodological rigor, and consensus alignment.
        """
        return [
    ...
    def from_list(cls, values: list[float]) -> "ConfidenceVector":
        """
        Creates a ConfidenceVector from a list of four floats.
        
        Args:
            values: List of four floats in the order: empirical support, theoretical basis, methodological rigor, consensus alignment.
        
        Returns:
            ConfidenceVector initialized with the provided values.
