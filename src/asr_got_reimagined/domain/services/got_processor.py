...
class GoTProcessor:
    ...
    def _initialize_stages(self) -> list[BaseStage]:
        """
        Initializes and returns the ordered list of processing stages for the ASR-GoT pipeline.
        
        Returns:
            A list of eight stage instances, each representing a distinct step in the query processing pipeline. Logs a warning if the number of initialized stages is not eight.
        """
    ...
    async def process_query(
        ...
        Initializes or continues a session, orchestrates the execution of all processing stages, logs detailed input and output information for each stage, handles errors (especially during initialization), and compiles the final results and metrics for the query.
    ...
    async def shutdown_resources(self):
        ...
        Currently, this method logs the shutdown event. No active resource management is performed here, but this is the intended place for cleanup logic if resource lifecycles are tied to the processor in the future.
        """
