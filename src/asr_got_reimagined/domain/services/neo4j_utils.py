...
def get_neo4j_settings() -> Neo4jSettings:
    """
    Retrieves the singleton Neo4jSettings instance, initializing it if not already created.
...
def execute_query(
    ...
    Args:
        """
    Initializes and returns a singleton Neo4j driver instance with authentication.
    
    If the driver is not already initialized or has been closed, this function creates a new driver using the configured URI and credentials, verifies connectivity, and returns the driver. Raises an exception if the connection cannot be established.
    """
