...
class SubgraphExtractionStage(BaseStage):
    ...
    def __init__(self, settings: Settings):
        ...
        The default criteria target high-confidence core nodes, key hypotheses and their support, and identified knowledge gaps, each with specific filters on confidence, impact, node types, and neighbor expansion depth.
        """
    ...
    def _build_cypher_conditions_for_criterion(self, criterion: SubgraphCriterion, params: Dict[str, Any]) -> List[str]:
        """
        Constructs Cypher WHERE clause conditions and populates query parameters based on the provided subgraph extraction criterion.
        
        Args:
            criterion: The subgraph extraction criterion specifying filters such as confidence, impact score, node types, layer IDs, knowledge gap status, and disciplinary tags.
            params: A dictionary to be populated with parameter values for the Cypher query.
        
        Returns:
            A list of Cypher WHERE clause condition strings corresponding to the filters in the criterion.
        """
        conditions: List[str] = []
    ...
    def _format_neo4j_node(self, neo4j_node_map: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If the input contains explicit "properties", "id", and "labels" keys, these are used directly. Otherwise, all keys except "id" and "labels" are treated as properties, with defaults applied as needed.
        
        Returns:
            A dictionary representing the node with "id", "labels", and "properties" fields.
        """
    ...
    def _format_neo4j_relationship(self, neo4j_rel_map: Dict[str, Any]) -> Dict[str, Any]:
        """
        Formats a Neo4j relationship map into a standardized dictionary with keys for ID, type, source and target node IDs, and properties.
        
        Args:
            neo4j_rel_map: A dictionary representing a Neo4j relationship, typically as returned by APOC procedures or Cypher queries.
        
        Returns:
            A dictionary with keys "id", "type", "source_id", "target_id", and "properties" representing the relationship in a consistent structure.
        """
    ...
    async def _extract_single_subgraph_from_neo4j(
        ...
        The method identifies seed nodes matching the criterion, expands to their neighbors up to the specified depth using APOC procedures, and collects all nodes and relationships within the resulting subgraph. The extracted data is formatted and returned as an `ExtractedSubgraphData` instance containing nodes, relationships, and extraction metrics.
        
        Args:
            criterion: The subgraph extraction criterion specifying filters and neighbor expansion depth.
    ...
    async def execute(
        ...
        """
        Executes the subgraph extraction stage by applying extraction criteria to Neo4j and aggregating results.
        
        This method retrieves subgraph extraction criteria from the session context (using defaults if none are provided), extracts subgraphs from Neo4j for each criterion, and aggregates the results. Only non-empty subgraphs are included. The output includes a summary, extraction metrics, and a context update containing the extracted subgraphs data for downstream processing.
        
        Args:
            current_session_data: The current session's data, including accumulated context and operational parameters.
        
        Returns:
            A StageOutput containing a summary, extraction metrics, and a context update with the extracted subgraphs.
        """
