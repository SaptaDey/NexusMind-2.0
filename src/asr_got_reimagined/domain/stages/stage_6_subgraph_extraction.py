...
class SubgraphExtractionStage(BaseStage):
    ...
    def __init__(self, settings: Settings):
        ...
        Defines a set of default subgraph extraction strategies, each specifying filters such as minimum confidence, impact score, node types, and neighbor expansion depth, to be used when custom criteria are not provided.
        """
    ...
    def _build_cypher_conditions_for_criterion(self, criterion: SubgraphCriterion, params: Dict[str, Any]) -> List[str]:
        ...
        Args:
            criterion: The subgraph extraction criterion specifying filters for node selection.
            params: Dictionary to be populated with Cypher parameter values for query binding.
    ...
    def _format_neo4j_node(self, neo4j_node_map: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If the input contains explicit "properties", "id", and "labels" keys (as returned by APOC queries), these are used directly. Otherwise, all keys except "id" and "labels" are treated as properties, with defaults provided if labels are missing.
        
        Returns:
            A dictionary with the node's ID, labels, and properties.
        """
    ...
    def _format_neo4j_relationship(self, neo4j_rel_map: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            neo4j_rel_map: A dictionary representing a Neo4j relationship, as returned by APOC procedures or Cypher queries.
    ...
    async def _extract_single_subgraph_from_neo4j(
        ...
        Finds seed nodes matching the criterion, expands the subgraph to include neighbors up to the specified depth using APOC procedures, and returns the extracted nodes and relationships in a standardized format. If no seed nodes are found, returns an empty subgraph with zero counts.
        
        Args:
            criterion: The subgraph extraction strategy specifying filters and neighbor expansion depth.
        
        Returns:
            An ExtractedSubgraphData object containing the subgraph's nodes, relationships, and extraction metrics.
        """
    ...
    async def execute(
        ...
        This method retrieves subgraph extraction criteria from the session context or uses defaults if none are provided. For each criterion, it asynchronously extracts a subgraph from Neo4j, collecting nodes and relationships that match the specified filters and neighbor expansion depth. Only non-empty subgraphs are included in the results. The method aggregates metrics across all extracted subgraphs, prepares a summary, and returns a StageOutput containing the summary, metrics, and a context update with the extracted subgraph data.
        """
