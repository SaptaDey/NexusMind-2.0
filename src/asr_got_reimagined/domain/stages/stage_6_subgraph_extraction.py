...
class SubgraphExtractionStage(BaseStage):
    ...
    def __init__(self, settings: Settings):
        ...
        The default criteria target high-confidence core nodes, key hypotheses with support, and identified knowledge gaps, each specifying node types, confidence and impact thresholds, and neighbor expansion depth.
        """
    ...
    def _build_cypher_conditions_for_criterion(self, criterion: SubgraphCriterion, params: Dict[str, Any]) -> List[str]:
        """
        Constructs Cypher WHERE clause conditions and query parameters from a subgraph extraction criterion.
        
        Args:
            criterion: The subgraph extraction criterion specifying filters for nodes.
            params: Dictionary to be populated with Cypher query parameters.
    ...
    def _format_neo4j_node(self, neo4j_node_map: Dict[str, Any]) -> Dict[str, Any]:
        ...
        If the input contains explicit 'properties', 'id', and 'labels' fields (as returned by APOC queries), these are used directly. Otherwise, all fields except 'id' and 'labels' are grouped under 'properties', with 'labels' defaulting to ['Node'] if not present.
    ...
    async def _extract_single_subgraph_from_neo4j(
        ...
        The method identifies seed nodes matching the criterion, expands the subgraph around them up to the specified neighbor depth using APOC procedures, and collects all nodes and relationships within the resulting subgraph. The extracted data is formatted into dictionaries suitable for downstream processing.
    ...
    async def execute(
        ...
        """
        Executes the subgraph extraction stage by applying extraction criteria to Neo4j and collecting subgraphs.
        
        Retrieves subgraph extraction criteria from the session context or uses defaults if none are provided. For each criterion, extracts a subgraph from Neo4j, formats the results, and aggregates metrics. Returns a StageOutput containing a summary, extraction metrics, and a context update with all extracted subgraphs.
        """
