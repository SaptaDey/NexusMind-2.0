...
class CompositionStage(BaseStage):
    ...
    async def _generate_executive_summary(
        ...
        Returns:
            A formatted executive summary string highlighting the number and names of key subgraphs.
        """
    ...
    async def _format_node_dict_as_claim(
        ...
        Returns:
            A tuple containing the formatted claim text referencing the node and a CitationItem with node metadata.
        """
    ...
    async def _generate_section_from_subgraph_data( # Renamed and signature changed
        ...
        Identifies and highlights up to three high-impact or high-confidence nodes (such as hypotheses, evidence, or interdisciplinary bridges) within the provided subgraph data. For each, formats a claim, collects citations, and summarizes their most relevant relationships. If no qualifying nodes are found, notes the absence of high-impact claims. Returns the composed section and associated citations.
        """
    ...
    async def _generate_reasoning_trace_appendix_summary(self, session_data: GoTProcessorSessionData) -> str:
        ...
        Iterates over the stage outputs trace and formats each stage's number, name, summary, and duration into a readable summary string.
        """
    ...
    async def execute(
        ...
        This method parses raw subgraph extraction results from the session context, validates and processes them into structured sections and citations, generates an executive summary and reasoning trace appendix, and assembles the final composed output. If no valid subgraphs are available, it returns a minimal output. The composed output and related metrics are returned in a StageOutput object, with the context updated for downstream processing.
