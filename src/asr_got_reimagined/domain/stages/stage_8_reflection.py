...
class ReflectionStage(BaseStage):
    ...
    def __init__(self, settings: Settings):
        ...
        Loads threshold parameters from default settings and defines the conceptual audit checks to be performed during the stage.
        """
    ...
    async def _check_high_confidence_impact_coverage_from_neo4j(self) -> AuditCheckResult:
        ...
        Queries relevant nodes from Neo4j, calculates the coverage ratios for nodes exceeding configured confidence and impact thresholds, and assigns a status based on whether these ratios meet predefined criteria. Returns an audit result indicating pass, warning, fail, or not applicable if no relevant nodes are found.
        """
        query = """
        MATCH (n:Node)
    ...
    async def _check_bias_flags_assessment_from_neo4j(self) -> AuditCheckResult:
        ...
        Queries all nodes with non-null bias flag metadata, counts the number of flagged nodes and those with high severity bias, and determines the audit status based on configured thresholds. Returns a failure result if an error occurs during database access or data parsing.
        """
        query = """
    ...
    async def _check_knowledge_gaps_addressed_from_neo4j(self, composed_output: Optional[ComposedOutput]) -> AuditCheckResult:
        ...
        If knowledge gap nodes are present in the graph, verifies that the composed output contains sections referencing gaps. Returns "PASS" if gaps are addressed, "WARNING" if not, "NOT_APPLICABLE" if no gaps exist, or "FAIL" if a query error occurs.
        """
    ...
    async def _check_hypothesis_falsifiability_from_neo4j(self) -> AuditCheckResult:
        ...
        Returns:
            An AuditCheckResult indicating whether the proportion of falsifiable hypotheses meets the configured threshold, with status "PASS", "WARNING", "FAIL", or "NOT_APPLICABLE" if no hypotheses are found. Returns "FAIL" if a Neo4j error occurs.
        """
        query = """
    ...
    async def _check_statistical_rigor_from_neo4j(self) -> AuditCheckResult:
        ...
        Queries all evidence nodes for statistical power data, counts those meeting the minimum power threshold, and evaluates the proportion against the configured requirement. Returns an audit result indicating whether the statistical rigor criterion is met, partially met, or failed due to insufficient data or query errors.
        """
        query = """
    ...
    async def _check_temporal_consistency(self) -> AuditCheckResult:
        """
        Returns an AuditCheckResult indicating that the temporal consistency check is not yet implemented.
        """
    ...
    async def _calculate_final_confidence(self, audit_results: List[AuditCheckResult]) -> ConfidenceVector:
        ...
        Adjusts the confidence dimensions according to the outcomes of hypothesis falsifiability, bias assessment, and statistical rigor checks. Each dimension is clamped between 0.0 and 1.0.
    ...
    async def execute(
        ...
        """
        Executes the reflection stage by running a series of audit checks on Neo4j graph data and composed output, then computes and returns a confidence assessment and audit summary.
        
        Performs multiple asynchronous audit checks, aggregates their results, calculates a final confidence vector, and prepares a summary and metrics for downstream processing.
        
        Args:
            current_session_data: The current session's data, including accumulated context and composed output.
        
        Returns:
            A StageOutput containing a summary of audit results, metrics, and updated context with the final confidence vector and audit check results.
        """
