...
class ReflectionStage(BaseStage):
    ...
    def __init__(self, settings: Settings):
        """
        Initializes the ReflectionStage with audit thresholds and checklist items from settings.
        
        Loads threshold parameters for confidence, impact, falsifiability, bias, and evidence power from default parameters. Sets up the list of audit checklist item names to be used in the stage.
        """
    ...
    async def _check_high_confidence_impact_coverage_from_neo4j(self) -> AuditCheckResult:
        """
        Performs an audit check to assess the proportion of nodes with high confidence and high impact in Neo4j.
        
        Queries relevant nodes and calculates the coverage ratios for those exceeding configured confidence and impact thresholds. Returns an audit result indicating PASS, WARNING, FAIL, or NOT_APPLICABLE based on these ratios. If a Neo4j error occurs, returns a FAIL status with the error message.
        """
        query = """
        MATCH (n:Node)
    ...
    async def _check_bias_flags_assessment_from_neo4j(self) -> AuditCheckResult:
        ...
        Queries all nodes with non-null bias flag metadata, counts the number of flagged nodes and those with high severity bias. Returns an audit result with status FAIL if high severity bias nodes exceed the configured maximum, WARNING if any bias flags are present, or PASS otherwise. Returns FAIL if a database, JSON parsing, or validation error occurs.
        """
        query = """
    ...
    async def _check_knowledge_gaps_addressed_from_neo4j(self, composed_output: Optional[ComposedOutput]) -> AuditCheckResult:
        ...
        Queries the database for nodes marked as knowledge gaps and determines if these gaps are mentioned in the provided composed output sections. Returns NOT_APPLICABLE if no knowledge gap nodes exist, PASS if gaps are addressed, WARNING if gaps are present but not explicitly mentioned, or FAIL if a database error occurs.
        
        Args:
            composed_output: The composed output object to check for references to knowledge gaps.
        
        Returns:
            An AuditCheckResult indicating whether knowledge gaps are addressed in the output.
        """
    ...
    async def _check_hypothesis_falsifiability_from_neo4j(self) -> AuditCheckResult:
        """
        Evaluates the proportion of hypotheses in Neo4j that include falsifiability criteria.
        
        Returns:
            An AuditCheckResult indicating PASS if the ratio of falsifiable hypotheses meets or exceeds the configured minimum, WARNING if some but not enough are falsifiable, FAIL if none are falsifiable, or NOT_APPLICABLE if no hypotheses are present.
        """
        query = """
    ...
    async def _check_statistical_rigor_from_neo4j(self) -> AuditCheckResult:
        """
        Evaluates the statistical rigor of evidence nodes in Neo4j based on statistical power.
        
        Queries all evidence nodes for statistical power information, counts those meeting the minimum power threshold (>= 0.7), and determines the proportion of adequately powered evidence. Returns a PASS if the ratio meets the configured minimum, WARNING otherwise, or FAIL if a Neo4j error occurs. If no evidence nodes are present, returns NOT_APPLICABLE.
        """
        query = """
    ...
    async def _check_temporal_consistency(self) -> AuditCheckResult:
        """
        Returns an audit check result indicating that the temporal consistency check is not yet implemented.
        """
    ...
    async def _check_collaboration_attributions(self) -> AuditCheckResult:
        """
        Returns an audit check result indicating that the collaboration attributions check is not yet implemented.
        """
    ...
    async def _calculate_final_confidence(self, audit_results: List[AuditCheckResult]) -> ConfidenceVector:
        """
        Calculates the final confidence vector based on the results of audit checks.
        
        The confidence vector's components are adjusted according to the statuses of relevant audit checks, with empirical support and methodological rigor updated based on statistical rigor, falsifiability, and bias assessment results. All values are clamped between 0.0 and 1.0.
        
        Args:
            audit_results: List of audit check results to inform confidence adjustments.
        
        Returns:
            A ConfidenceVector reflecting the overall assessment of the graph data.
        """
    ...
    async def execute(
        ...
        """
        Executes the reflection stage by running a series of audit checks on Neo4j graph data and computing a final confidence vector.
        
        Runs multiple asynchronous audit checks, aggregates their results, and calculates an overall confidence assessment. Produces a summary, metrics, and context updates for downstream processing.
        
        Args:
            current_session_data: The session data containing accumulated context and outputs from previous stages.
        
        Returns:
            A StageOutput object containing a summary of audit results, metrics, and updated context with the final confidence vector and audit check results.
        """
