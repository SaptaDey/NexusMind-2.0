...
def test_initialize_endpoint() -> Dict[str, Any]:
    """
    Sends a JSON-RPC initialize request to the MCP server and verifies the response.
    
    Returns:
        The parsed JSON response from the server if the request is successful and valid,
        or a dictionary containing error details if the request fails or the response is invalid.
    """
...
def test_asr_got_query(session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    Args:
        session_id: An optional session identifier to include in the request. If not provided, a default value is used.
    
    Returns:
        The parsed JSON response from the server if the request is successful, or a dictionary containing error details if the request fails or the response is invalid.
    """
...
def test_shutdown() -> Dict[str, Any]:
    """
    Sends a JSON-RPC shutdown request to the MCP server and returns the server's response.
...
def run_all_tests():
    """
    Runs the MCP server endpoint test suite sequentially.
    
    Executes the initialization test, and if successful, performs the ASR query test using a generated session ID. Prints a completion message after all tests.
    """
