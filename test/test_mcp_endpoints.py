...
def test_initialize_endpoint() -> Dict[str, Any]:
    """
    Sends a JSON-RPC initialize request to the MCP server and checks the response for validity.
    
    Returns:
        The parsed JSON response from the server if the request is successful and the response is valid, or a dictionary with error details if the request fails or the response is invalid.
    """
...
def test_asr_got_query(session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    Args:
        session_id: An optional session identifier to include in the request. If not provided, a default session ID is used.
...
def run_all_tests():
    """
    Runs the MCP endpoint test suite, including initialization and query tests.
